...
- Showcase path (GitHub Pages, v0.46.2): `bun run --cwd packages/web build:demo` → `dist-demo/`
  (replay at `/`, engineering map at `/engineering/`); `pages.yml` builds it on push. v0.48.1: the
  tape is per language — `packages/web/demo/{en,zh}/` (script, voice, her diaries + skills), shared
  `demo/data/` (dreams, settings) + `demo/music/`; `script.test.ts` pins zh to en's skeleton. New
  lines need `bun run --cwd packages/web render:voice --lang <en|zh>` against a throwaway api_v2
  (never his 9880).
